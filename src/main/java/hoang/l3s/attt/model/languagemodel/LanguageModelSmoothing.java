...
public class LanguageModelSmoothing {
	...
	public double caculateProb(int preTermCount, int termCount, double bgProb, int newTermInFgCount,
			SmoothingType type) {
		...
		case NoSmoothing:
			prob = ((double) termCount) / preTermCount;
	 ...
	 */
	public int caculateNewTermCountInFg(HashMap<String, HashMap<String, Integer>> fgnTermCountMap,
			HashMap<String, HashMap<String, Double>> bgProbMap) {
		...
		while (iter.hasNext()) {
			...
			HashMap<String, Integer> fgTermCount = fgnTermCountMap.get(preTerm);
	 ...
	 * Prob(w| B.ML)
	 */
	public void traverseFgLM(HashMap<String, Integer> fgPreTermCountMap,
			HashMap<String, HashMap<String, Integer>> fgnTermCountMap,
			HashMap<String, HashMap<String, Double>> bgProbMap, SmoothingType smoothingType, int newTermCountInFg) {
		...
		while (iter.hasNext()) {
			String preTerm = iter.next();
			HashMap<String, Integer> termUnderPreTermCountMap = fgnTermCountMap.get(preTerm);
			Set<String> keys = termUnderPreTermCountMap.keySet();
			Iterator<String> iter1 = keys.iterator();
			while (iter1.hasNext()) {
				String term = iter1.next();

				int preTermCount = fgPreTermCountMap.get(preTerm);
				int termCount = fgnTermCountMap.get(preTerm).get(term);

				double bgProb = getBgProb(preTerm, term, bgProbMap);

				double prob = caculateProb(preTermCount, termCount, bgProb, newTermCountInFg, smoothingType);
				updateBgLM(preTerm, term, prob, bgProbMap);
				System.out.println(
						"find in F.LM:" + formatPrintProb(Configure.nGram, preTerm, term, preTermCount, termCount, prob)
								+ " with bgProb:" + bgProb);
				num++;
			}
		}
	...
	/*
	 * traverse B.LM if "w" is in F.LM, do nothing, because we have processed
	 * when traversing F.LM if "w" is not in F.LM, modify the Prob(w| B.ML)
	 */
	public void traverseBgLM(HashMap<String, Integer> fgPreTermCountMap,
			HashMap<String, HashMap<String, Integer>> fgnTermCountMap,
		...
		while (iter.hasNext()) {
			...
			while (iter1.hasNext()) {
				...
				updateBgLM(preTerm, term, prob, bgProbMap);
				System.out.println(
						"find in B.LM:" + formatPrintProb(Configure.nGram, preTerm, term, preTermCount, termCount, prob)
								+ " with bgProb:" + bgProb);
				num++;
			}
		}
	...
	/*
	 * To reslove the problem the sum of the prob will not be 1 after some
	 * smoothing technology, we use normalize.
	 * 
	 * Becasue the function traverseFgLM() and traverseBgLM() maybe ignore some
	...
	public void smoothing(HashMap<String, Integer> fgPreTermCountMap,
			HashMap<String, HashMap<String, Integer>> fgnTermCountMap,
			HashMap<String, HashMap<String, Double>> bgProbMap, SmoothingType smoothingType) {
		...
		traverseFgLM(fgPreTermCountMap, fgnTermCountMap, bgProbMap, smoothingType, newTermCountInFg);

		if (smoothingType == SmoothingType.NoSmoothing)
			return;
		traverseBgLM(fgPreTermCountMap, fgnTermCountMap, bgProbMap, smoothingType, newTermCountInFg);
