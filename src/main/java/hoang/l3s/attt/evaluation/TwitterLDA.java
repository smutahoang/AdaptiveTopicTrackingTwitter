...
public class TwitterLDA {
	...
	private void sampleTweetTopic(int e, int t) {
		// sample the topic for tweet number t of epoch number e
		// get current topic
		int currz = epoches[e].tweets[t].topic;
		n_ze[currz][e]--;
		sum_nze[e]--;
		for (int i = 0; i < epoches[e].tweets[t].words.length; i++) {
			if (epoches[e].tweets[t].coins[i] == 0)
		...
		for (int z = 0; z < nTopics; z++) {
			p[z] = (n_ze[z][e] + alpha) / (sum_nze[e] + sum_alpha);
			for (int i = 0; i < epoches[e].tweets[t].words.length; i++) {
				if (epoches[e].tweets[t].coins[i] == 0)
					continue;// do not consider background words
				int w = epoches[e].tweets[t].words[i];
	...
	private double getTweetLikelihood(int e, int t) {
		...
		for (int i = 0; i < epoches[e].tweets[t].words.length; i++) {
			...
			double p_1 = 0;
			for (int z = 0; z < nTopics; z++) {
				double p_z = tweetTopics[z][w] * epoches[e].topicDistribution[z];
				p_1 = p_1 + p_z;
			}
			p_1 = p_1 * coinBias[1];
	...
	private void inferTweetTopic() {
		for (int e = 0; e < epoches.length; e++) {
			for (int t = 0; t < epoches[e].tweets.length; t++) {
				...
				for (int z = 0; z < nTopics; z++) {
					posteriors[z] = getTweetLikelihood(e, t, z) + Math.log10(epoches[e].topicDistribution[z]);
				}

				epoches[e].tweets[t].inferedTopic = -1;// background topic only
				epoches[e].tweets[t].inferedLikelihood = Double.NEGATIVE_INFINITY;
				double sum = 0;
				for (int z = 0; z < nTopics; z++) {
					if (posteriors[z] > epoches[e].tweets[t].inferedLikelihood) {
						epoches[e].tweets[t].inferedLikelihood = posteriors[z];
						epoches[e].tweets[t].inferedTopic = z;
					}
					posteriors[z] = Math.pow(10, posteriors[z]);
					sum += posteriors[z];
				}
				epoches[e].tweets[t].inferedPosteriorProb = posteriors[epoches[e].tweets[t].inferedTopic] / sum;
			}
		}
	}
	...
	private void outputInferedTweetTopic() {
		try {
			...
			for (int e = 0; e < epoches.length; e++) {
				for (int t = 0; t < epoches[e].tweets.length; t++)
					bw.write(String.format("%d\t%s\t%d\t%f\t%f\n", e, epoches[e].tweets[t].tweetID,
							epoches[e].tweets[t].inferedTopic, epoches[e].tweets[t].inferedLikelihood,
							epoches[e].tweets[t].inferedPosteriorProb));
			}
			bw.close();
		} catch (Exception e) {
			System.out.println("Error in writing out tweet topics to file!");
	...
	public static void main(String[] args) {
		...
		String[] paths = new String[] { "keyword_topImportant", "pseudo_supervised", "graph" };
		String[] models = new String[] { "kw", "ps", "graph" };
		int[] nTopics = new int[] { 10, 20, 30 };
		for (int m = 0; m < 1; m++) {
			String dataPath = String.format("/home/hoang/attt/output/%s/ua/usScandal_2017-04-10_%sFilteredTweets.txt",
					paths[m], models[m]);
			for (int i = 0; i < 3; i++) {
				String outputPath = String.format("/home/hoang/attt/evaluation/ua/%s/%d", models[m], nTopics[i]);
				TwitterLDA twitterLDA = new TwitterLDA(dataPath, outputPath, nTopics[i]);
				twitterLDA.readData();
