...
public class AliasSampler {
	...
	public AliasSampler(Random _rand) {
		aliases = null;
		nOutcomes = -1;
		outcomes = null;
		flag = false;
		...
		while (true) {
			if (currentL < lCount) {
				if (currentH >= hCount) {// last element, no more high vase
					aliases[aIndex] = new Alias(lIndexes[currentL], lIndexes[currentL], lProbs[currentL]);
					aIndex++;
					currentL++;
				} else {
					aliases[aIndex] = new Alias(lIndexes[currentL], hIndexes[currentH], lProbs[currentL]);
					aIndex++;
				...
				aliases[aIndex] = new Alias(hIndexes[currentH], hIndexes[currentH], hProbs[currentH]);
				aIndex++;
				currentH++;
			} else {
				break;
	 ...
	 * @param r
	 */
	public AliasSampler(int[] outcomeIndexes, double[] outcomeLikelihoods, int nElements) {
		nOutcomes = nElements;
	...
	public int sample() {
		if (nOutcomes == 1) {
			...
			int o = -1;
			int bin = rand.nextInt(nOutcomes);
			if (nOutcomes * aliases[bin].prob > rand.nextDouble()) {
